...
Advanced EEG-Based Biometric Security with Wave Visualization
"""
...
class BrainPasswordCore:
        ...
        try:
            if self.serial_conn and self.serial_conn.is_open and self.serial_conn.in_waiting:
                ...
                if line:
                    ...
                    self.realtime_buffer.append(value)
    ...
    def get_realtime_buffer(self):
        ...
        return np.array(self.realtime_buffer) if self.realtime_buffer else np.array([])
    ...
    def extract_features(self, eeg_data):
        ...
        features.extend([
            ...
            len(np.where(np.diff(np.sign(eeg_data)))[0]),  # Zero crossings
        ])
    ...
    def train_model(self, task1_data_list, task2_data_list):
        ...
        # Store templates (average of training samples)
        self.task1_template = np.mean(task1_features, axis=0)
        self.task2_template = np.mean(task2_features, axis=0)
        
        # Prepare training data
        # Positive class: correct sequence of tasks
        X_positive = np.vstack(task1_features + task2_features)
        y_positive = np.ones(len(X_positive))
        
        # Generate negative samples (representing wrong mental states)
        X_negative = []
        n_negative = len(X_positive) * 3  # More negative samples for better discrimination
        ...
        for _ in range(n_negative):
            ...
            if choice == 0:
                ...
                # Shuffled features (wrong pattern)
                ...
                # Heavily corrupted positive sample
                neg_sample = X_positive[np.random.randint(0, len(X_positive))].copy()
                noise = np.random.normal(0, np.std(X_positive) * 1.5, neg_sample.shape)
    ...
    def authenticate_data(self, auth_data_list):
        ...
        Only grants access if BOTH tasks match the trained pattern.
        """
        ...
        # Extract features from both auth tasks
        auth_features = []
        ...
        # Extract confidence scores for positive class
        confidences = probabilities[:, 1] if probabilities.shape[1] > 1 else probabilities[:, 0]
        
        # Calculate similarity to templates (cosine similarity)
        ...
        # STRICT AUTHENTICATION CRITERIA:
        # 1. Both tasks must be predicted as positive class
        # 2. Both confidence scores must be high (>0.60)
        # 3. Both template similarities must be high (>0.65)
        # 4. Combined confidence must be very high
        ...
        # Success only if ALL criteria met
...
class EEGVisualizationWindow:
    """Real-time EEG wave visualization window"""
    ...
    def setup_ui(self):
        ...
        # Frequency domain plot (band powers)
    ...
    def start_visualization(self):
        ...
        # Remove blit=True and use a proper update mechanism
        self.ani = animation.FuncAnimation(
            self.fig,
            self.update_plot,
            interval=50,  # Faster update - every 50ms
            blit=False,
            cache_frame_data=False
        )
        # Important: Start the animation properly with tkinter
        self.canvas.draw()
    
    def update_plot(self, frame):
        """Update plots with new data"""
        if not self.is_running or not self.core.is_connected:
            return
        ...
        try:
            ...
            if len(data) > 10:  # Need minimum data points
                ...
                self.ax1.clear()
                self.ax1.plot(range(len(data)), data, color='#00ff9f', linewidth=1.5)
                self.ax1.set_title('EEG Signal (Time Domain)', color='#00ff9f', fontsize=12, fontweight='bold')
                self.ax1.set_xlabel('Sample', color='#ffffff')
                self.ax1.set_ylabel('Amplitude', color='#ffffff')
                self.ax1.tick_params(colors='#ffffff')
                self.ax1.grid(True, alpha=0.3, color='#ffffff')
                self.ax1.set_facecolor('#1a1f3a')
...
class BrainPasswordGUI:
    ...
    def on_close(self):
        ...
        try:
            self.log("Shutting down system...", '#ffa502')
        except:
            pass
    ...
    def setup_ui(self):
        ...
        # ADD THIS: Force periodic updates
        self.update_display()

    def update_display(self):
        """Periodic update to keep GUI responsive"""
